...
public class PerfilDocenteController {
    ...
    public PerfilDocenteController(UsuarioDAO usuarioDAO, FotoPerfilService fotoPerfilService) {
        this.usuarioDAO = usuarioDAO; this.fotoPerfilService = fotoPerfilService;
    }

    @PostMapping("/docente/perfil/foto")
    ...
    @GetMapping("/docente/perfil/foto")
    ...
    private Usuario usuario(Authentication authentication) {
        return usuarioDAO.buscarPorEmail(authentication.getName()).orElseThrow(() -> new IllegalArgumentException("Usuario no encontrado."));
    }
}
